...
% successors, which are binary trees themselves. % In Prolog we represent the
% empty tree by the atom 'nil' and the non-empty tree by the term t(X,L,R),
% where X denotes the root node and L and R denote the left and right subtree,
...
% % 4.01 (*) Check whether a given term represents a binary tree % Write a
% predicate istree/1 which succeeds if and only if its argument is a Prolog
% term representing a binary tree. % Example: % ?-
% istree(t(a,t(b,nil,nil),nil)). % Yes % ?- istree(t(a,t(b,nil,nil))). % No

% % 4.02 (**) Construct completely balanced binary trees % In a completely
% balanced binary tree, the following property holds for every node: The number
% of nodes in its left subtree and the number of nodes in its right subtree are
...
% Example: % ?- cbal_tree(4,T). % T = t(x, t(x, nil, nil), t(x, nil, t(x, nil,
% nil))) ; % T = t(x, t(x, nil, nil), t(x, t(x, nil, nil), nil)) ; %
% etc......No

% % 4.03 (**) Symmetric binary trees % Let us call a binary tree symmetric if
% you can draw a vertical line through the root node and then the right subtree
% is the mirror image of the left subtree. Write a predicate symmetric/1 to
% check whether a given binary tree is symmetric. Hint: Write a predicate
% mirror/2 first to check whether one tree is the mirror image of another. We
% are only interested in the structure, not in the contents of the nodes.

% % 4.04 (**) Binary search trees (dictionaries) % Use the predicate add/3,
% developed in chapter 4 of the course, to write a predicate to construct a
% binary search tree from a list of integer numbers. % Example: % ?-
% construct([3,2,5,7,1],T). % T = t(3, t(2, t(1, nil, nil), nil), t(5, nil,
% t(7, nil, nil)))

% % Then use this predicate to test the solution of the problem P56. % Example:
% % ?- test_symmetric([5,3,18,1,4,12,21]). % Yes % ?-
% test_symmetric([3,2,5,7,4]). % No

% % 4.05 (**) Generate-and-test paradigm % Apply the generate-and-test paradigm
% to construct all symmetric, completely balanced binary trees with a given
% number of nodes. Example: % ?- sym_cbal_trees(5,Ts). % Ts = [t(x, t(x, nil,
% t(x, nil, nil)), t(x, t(x, nil, nil), nil)), t(x, t(x, t(x, nil, nil), nil),
% t(x, nil, t(x, nil, nil)))]
...
% % 4.06 (**) Construct height-balanced binary trees % In a height-balanced
% binary tree, the following property holds for every node: The height of its
% left subtree and the height of its right subtree are almost equal, which
% means their difference is not greater than one.

% % Write a predicate hbal_tree/2 to construct height-balanced binary trees for
% a given height. The predicate should generate all solutions via backtracking.
% Put the letter 'x' as information into all nodes of the tree. % Example: % ?-
% hbal_tree(3,T). % T = t(x, t(x, t(x, nil, nil), t(x, nil, nil)), t(x, t(x,
% nil, nil), t(x, nil, nil))) ; % T = t(x, t(x, t(x, nil, nil), t(x, nil,
% nil)), t(x, t(x, nil, nil), nil)) ; % etc......No

% % 4.07 (**) Construct height-balanced binary trees with a given number of
% nodes % Consider a height-balanced binary tree of height H. What is the
% maximum number of nodes it can contain? % Clearly, MaxN = 2**H - 1. However,
% what is the minimum number MinN? This question is more difficult. Try to find
% a recursive statement and turn it into a predicate minNodes/2 defined as
% follwos:
...
% % 4.08 (*) Count the leaves of a binary tree % A leaf is a node with no
% successors. Write a predicate count_leaves/2 to count them.

% % % count_leaves(T,N) :- the binary tree T has N leaves

% % 4.09 (*) Collect the leaves of a binary tree in a list % A leaf is a node
% with no successors. Write a predicate leaves/2 to collect them in a list.

% % % leaves(T,S) :- S is the list of all leaves of the binary tree T

% % 4.10 (*) Collect the internal nodes of a binary tree in a list % An
% internal node of a binary tree has either one or two non-empty successors.
% Write a predicate internals/2 to collect them in a list.

% % % internals(T,S) :- S is the list of internal nodes of the binary tree T.

% % 4.11 (*) Collect the nodes at a given level in a list % A node of a binary
% tree is at level N if the path from the root to the node has length N-1. The
% root node is at level 1. Write a predicate atlevel/3 to collect all nodes at
% a given level in a list.
...
% % 4.12 (**) Construct a complete binary tree % A complete binary tree with
% height H is defined as follows: The levels 1,2,3,...,H-1 contain the maximum
% number of nodes (i.e 2**(i-1) at the level i, note that we start counting the
...
% % 4.13 (**) Layout a binary tree (1) % Given a binary tree as the usual
% Prolog term t(X,L,R) (or nil). As a preparation for drawing the tree, a
% layout algorithm is required to determine the position of each node in a
% rectangular grid. Several layout methods are conceivable, one of them is
...
% following two rules: % x(v) is equal to the position of the node v in the
% inorder % y(v) is equal to the depth of the node v in the tree % sequence %
...
% R % Write a predicate layout_binary_tree/2 with the following specification:

% % % layout_binary_tree(T,PT) :- PT is the "positioned" binary tree obtained
...
% % Use the same conventions as in problem 4.13 and test your predicate in an
% appropriate way.
...
% % Yet another layout strategy is shown in the above illustration. The method
...
% predicate in an appropriate way. Note: This is a difficult problem. Don't
% give up too early!
...
% % b) Write the same predicate tree_string/2 using difference lists and a
% single predicate tree_dlist/2 which does the conversion between a tree and a
...
% and inorder sequence of a given binary tree, respectively. The results should
% be atoms, e.g. 'abdecfg' for the preorder sequence of the example in problem
% 4.16.
...
% % 4.18 (**) Dotstring representation of binary trees% We consider again binary
%  trees with nodes that are identified by single lower-case letters, as in the 
%  example of problem 4.16. Such a tree can be represented by the preorder 
%  sequence of its nodes in which dots (.) are inserted where an empty 
%  subtree (nil) is encountered during the tree traversal.
...
%   predicate tree_dotstring/2 which does the conversion in both directions.
