...
% Prolog term: T = t(a,[t(f,[t(g,[])]),t(c,[]),t(b,[t(d,[]),t(e,[])])]) 5.01
% (*) Check whether a given term represents a multiway tree Write a predicate
% istree/1 which succeeds if and only if its argument is a Prolog term
% representing a multiway tree. Example: ?-
% istree(t(a,[t(f,[t(g,[])]),t(c,[]),t(b,[t(d,[]),t(e,[])])])). Yes

% 5.02 (*) Count the nodes of a multiway tree Write a predicate nnodes/1 which
% counts the nodes of a given multiway tree. Example: ?-
% nnodes(t(a,[t(f,[])]),N). N = 2

% Write another version of the predicate that allows for a flow pattern (o,i).

% 5.03 (**) Tree construction from a node string We suppose that the nodes of a
% multiway tree contain single characters. In the depth-first order sequence of
% its nodes, a special character ^ has been inserted whenever, during the tree
...
% 5.04 (*) Determine the internal path length of a tree We define the internal
% path length of a multiway tree as the total sum of the path lengths from the
% root to all nodes of the tree. By this definition, the tree in the figure of
% problem 5.03 has an internal path length of 9.

% Write a predicate ipl(Tree,IPL) for the flow pattern (+,-).

% 5.05 (*) Construct the bottom-up order sequence of the tree nodes Write a
% predicate bottom_up(Tree,Seq) which constructs the bottom-up sequence of the
% nodes of the multiway tree Tree. Seq should be a Prolog list.

% What happens if you run your predicate backwords?

% 5.06 (**) Lisp-like tree representation There is a particular notation for
% multiway trees in Lisp. Lisp is a prominent functional programming language,
% which is used primarily for artificial intelligence problems. As such it is
% one of the main competitors of Prolog. In Lisp almost everything is a list,
...
% ')']. Write a predicate tree_ltl(T,LTL) which constructs the "lispy token
% list" LTL if the tree is given as term T in the usual Prolog notation.

% Example: ?- tree_ltl(t(a,[t(b,[]),t(c,[])]),LTL). LTL = ['(', a, '(', b, c,
% ')', ')']

% As a second, even more interesting exercise try to rewrite tree_ltl/2 in a
